...
Invoke-WebRequest -Uri 'https://api.cloudmersive.com/multi-threat-scan/v1/swagger.json' -OutFile '.\multithreat-api-swagger.json'
(Get-Content .\multithreat-api-swagger.json).replace('localhost', "api.cloudmersive.com") | Set-Content .\multithreat-api-swagger.json
(Get-Content .\multithreat-api-swagger.json).replace('"http"', '"https"') | Set-Content .\multithreat-api-swagger.json

java -jar ./openapi-generator-cli-7.12.0.jar generate -i multithreat-api-swagger.json -g csharp -o client -c packageconfig.json
...
function Test-StrongNameDiagnostic {
    ...
    try {
        ...
        Write-Host "PublicKey size : $($pubKey.Length) bytes (288 = typical 2048-bit RSA public key blob)"
    } catch {
